...
export default function ProfilePhotos() {
    const { id } = useParams();
    const { photos, loadingPhotos, isCurrentUser, uploadPhoto, setMainPhoto, deletePhoto, profile } = useProfile(id);
    ...
    if(!photos) return <Typography>No photo found for this user</Typography>
    ...
    return (
        <Box>
            ...
            <Divider sx={{ my: 2 }} />
            { editMode 
                ? <PhotoUploadWidget uploadPhoto={handlePhotoUpload} loading={loadingPhotos} />
                : photos.length === 0 
